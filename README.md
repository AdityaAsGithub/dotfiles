...
| -------|---------|---------------|
| `GH_USERNAME` | The github username | "AdityaAsGithub" |
...
| `KEYBASE_USERNAME` | Keybase username | "aditan2003" |
...
1. **_Only in this README file and `files/.zprofile` files (and nowhere else):_** Find and replace the strings that reference my usernames to your equivalent ones (for eg, you can search for `AdityaAsGithub` and `aditan2003` and replace them with your values).
...
```zsh
export GH_USERNAME="AdityaAsGithub"; export DOTFILES_DIR="${HOME}/.bin-oss"; curl -fsSL "https://raw.githubusercontent.com/${GH_USERNAME}/dotfiles/master/scripts/fresh-install-of-osx.sh" | zsh
```
